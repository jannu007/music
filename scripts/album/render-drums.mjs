/**
 * 天問 (Tenmon) アルバム — ドラム・パート WAV 書き出しスクリプト
 *
 * drums アプリの Demo UI とは完全に独立した /album/harness-drums.html を
 * ヘッドレス Chromium で開き、tenmon-01..tenmon-10 の10曲ぶんのドラム・
 * パートをオフラインレンダリングして WAV として書き出す。
 *
 *   node scripts/album/render-drums.mjs
 */
...
async function main() {
  ...
  try {
    ...
    for (const id of TRACK_IDS) {
      ...
      await page.goto(`${BASE_URL}/album/harness-drums.html?track=${id}`, { waitUntil: 'load' });
      ...
      const outPath = resolve(trackDir, 'drums.wav');

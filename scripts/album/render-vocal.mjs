/**
 * 天問 (Tenmon) アルバム — ボーカル・パート（スキャット）WAV 書き出しスクリプト
 *
 * vocal アプリの Demo UI とは完全に独立した /album/harness-vocal.html を
 * ヘッドレス Chromium で開き、tenmon-01..tenmon-10 の10曲ぶんのスキャット・
 * パートをオフラインレンダリングして WAV として書き出す。
 *
 *   node scripts/album/render-vocal.mjs
 */
...
async function main() {
  ...
  // npx 経由だとラッパープロセスだけ終了して実体の vite が孫プロセスとして
  // 残ることがあるため、ローカルの vite バイナリを直接起動する（終了時に
  // 確実に kill できるようにするため）。
  ...
  try {
    ...
    for (const id of TRACK_IDS) {
      ...
      await page.goto(`${BASE_URL}/album/harness-vocal.html?track=${id}`, { waitUntil: 'load' });
      ...
      const outPath = resolve(trackDir, 'vocal.wav');

export interface MidiHandlers {
  ...
  sostenuto: (value: number) => void;
  soft: (value: number) => void;
  allNotesOff: () => void;
}
...
export class MidiInput {
  ...
  private onMessage(data: Uint8Array | number[]) {
    ...
    if (status === 0x90 && d2 > 0) {
      ...
      switch (d1) {
        case 64: this.handlers.sustain(d2 / 127); break;
        case 66: this.handlers.sostenuto(d2 / 127); break;
        case 67: this.handlers.soft(d2 / 127); break;
        case 120:
...
/** PCキーボードの配列（下段=低いオクターブ / 上段=1オクターブ上） */

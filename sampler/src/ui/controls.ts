...
export function stepper(
  ...
  minus.type = 'button';
  plus.type = 'button';
  const input = el('input', 'stepper-input');
  input.type = 'number';
...
export function section(title: string, hint?: string): HTMLElement {
  const root = el('div', 'panel-section');
  const head = el('div', 'panel-section-head');
  head.append(el('h3', '', title));
  if (hint) head.append(el('span', 'panel-hint', hint));
  root.append(head);
  return root;
}

export function grid(className = ''): HTMLElement {
  return el('div', `ctl-grid ${className}`.trim());
}
